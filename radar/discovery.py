...
def discover_links(page: Page, domains: list[str], terms: list[str], max_links: int = 25) -> list[dict]:
    ...
    for a in page.soup.find_all("a", href=True):
        ...
        if any(h in hay for h in ("job", "vacan", "phd", "doctoral", "position", "opportun")):
            score += 2
        if score <= 0:
            continue
        seen.add(url)
...
def structured_candidates(page: Page) -> list[dict]:
    """Return JobPosting JSON-LD records already extracted by the HTTP layer."""
    return list(page.structured_jobs or [])
