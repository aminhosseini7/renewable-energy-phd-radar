...
DATE_PATTERNS = [
    r"(?:deadline|closing date|applications? close|apply by|close date|closes)\s*[:\-]?\s*([0-3]?\d\s+[A-Za-z]{3,9}\s+[’']?\d{2,4})",
    r"(?:deadline|closing date|applications? close|apply by|close date|closes)\s*[:\-]?\s*([A-Za-z]{3,9}\s+[0-3]?\d(?:st|nd|rd|th)?[,]?\s+\d{4})",
    r"(?:deadline|closing date|applications? close|apply by|close date|closes)\s*[:\-]?\s*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})",
]
...
def extract_funding(text: str, dna: dict) -> dict:
    ...
    money_patterns = [
        r"(?:AUD|CAD|USD|EUR|GBP|A\$|C\$|\$|€|£)\s?\d{1,3}(?:,\d{3})+(?:\.\d+)?(?:\s*(?:per annum|per year|p\.a\.|pa|per month|monthly|annually))?",
